...
def pick_random_biome1():
    ...
    choice = secrets.choice(possible_biomes)
...
def generate_lot_value_for_biome(biome_type: str):
    if biome_type == "NONE":
        ...
        return secrets.choice(range(1,100))
def generate_area(min, max, k):
    return(random.sample(range(min, max), k))
def generate_area_list():
    global current_filename
    areas = []
    if 'su2' in current_filename:
        areas = generate_area(1, 5, 1)
    else:
        areas = generate_area(1, 28, 10)
    ret = ""
    for area in areas:
        ret = ret + str(area) +','
    return ret
...
legends = [998, 999, 493, 487, 483, 484, 382, 383, 384, 150, 151, 144, 146, 145, 720, 721, 719, 889, 888, 890, 897, 896, 898, 894, 895, 893, 995, 996, 997, 998, 891, 892, 642, 645, 641, 648, 482, 481, 480, 801, 905, 1011, 1334, 1335, 1336, 489, 490, 491, 492, 1014, 1015, 1016, 385, 243, 244, 245, 249, 250, 380, 381, 638, 639, 640, 643, 644, 646, 791, 792, 800, 1021, 1022, 386]
recreated_species = []
recreated_altforms = []
chosen_biomes = []
current_filename = ""
...
def randomize(config, filename):
    ...
    file = open(os.getcwd() + "/Randomizer/WildEncounters/" + filename + "_clean.json", "r")
    data = json.load(file)
    ...
    # clear global arrays
    recreated_species.clear()
    recreated_altforms.clear()
    ...
    data['values'] = [] #remove all existing entries
    ...
    for index in allowed_species:
        if config['exclude_legendaries'] == "yes":
           pass
            ...
            if index not in recreated_species:
                new_template = template_entry.copy()
                recreated_species.append(index)
                form = 0
                if index > 1025: #thats an alt form
                    form = alt_forms.get(index)["form"]
                    index = alt_forms.get(index)["species"]
                new_template = make_template(new_template, index, csvdata, form)
                data['values'].append(new_template)
    
    outdata = json.dumps(data, indent=4)
    with open(os.getcwd() + "/Randomizer/WildEncounters/" + filename + ".json", 'w') as outfile:
